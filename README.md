...
- [Overview](#overview)
- [Features](#features)
- [Input Process Details](#input-process-details)
- [How It Works](#how-it-works)
- [Gantt Chart and Times Display](#gantt-chart-and-times-display)
- [Usage](#usage)
...
1. **Process Arrival**: As the time progresses, processes are added to the appropriate queue based on their priority and arrival time.
2. **Execution**: The CPU selects a process from the highest priority queue that has available processes.
3. **Preemption**: If a process uses up its allotted time slice without completing, it is demoted to the next lower-priority queue.
4. **Completion**: Once a process completes, it is removed from the queue, and its **turnaround** and **waiting** times are calculated.
...
4. The Gantt chart and process waiting/turnaround times will be displayed in the console.
